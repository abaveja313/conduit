/* tslint:disable */
/* eslint-disable */
export const memory: WebAssembly.Memory;
export const add_files_to_staging: (a: number, b: number, c: number, d: number, e: number, f: number, g: number, h: number, i: number, j: number) => [number, number, number];
export const promote_staged_index: () => [number, number, number];
export const begin_index_staging: () => [number, number];
export const get_staging_info: () => [number, number, number];
export const commit_index_staging: () => [number, number, number];
export const revert_index_staging: () => [number, number];
export const get_staged_modifications: () => [number, number, number];
export const get_staged_deletions: () => [number, number, number];
export const get_modified_files_summary: () => [number, number, number];
export const get_file_diff: (a: number, b: number) => [number, number, number];
export const get_staged_modifications_with_active: () => [number, number, number];
export const abort_file_load: () => [number, number];
export const clear_wasm_index: () => [number, number];
export const validate_can_edit_lines: (a: number, b: number) => [number, number, number];
export const record_file_read: (a: number, b: number) => [number, number];
export const mark_file_needs_read: (a: number, b: number) => [number, number];
export const check_file_needs_read: (a: number, b: number) => [number, number, number];
export const create_index_file: (a: number, b: number, c: number, d: number) => [number, number, number];
export const delete_file: (a: number, b: number) => [number, number, number];
export const copy_file: (a: number, b: number, c: number, d: number) => [number, number, number];
export const copy_files: (a: any) => [number, number, number];
export const move_file: (a: number, b: number, c: number, d: number) => [number, number, number];
export const move_files: (a: any) => [number, number, number];
export const debug_file_info: (a: number, b: number, c: number) => [number, number, number];
export const debug_list_all_files: (a: number, b: number) => [number, number, number];
export const replace_lines: (a: number, b: number, c: any, d: number) => [number, number, number];
export const delete_lines: (a: number, b: number, c: number, d: number, e: number) => [number, number, number];
export const insert_before_line: (a: number, b: number, c: number, d: number, e: number, f: number) => [number, number, number];
export const insert_after_line: (a: number, b: number, c: number, d: number, e: number, f: number) => [number, number, number];
export const insert_lines: (a: number, b: number, c: any, d: number) => [number, number, number];
export const read_file_lines: (a: number, b: number, c: number, d: number, e: number) => [number, number, number];
export const init: () => void;
export const ping: () => [number, number];
export const file_count: () => number;
export const get_index_stats: () => [number, number, number];
export const clear_index: () => [number, number];
export const reset_all_indices: () => [number, number];
export const begin_file_load: () => [number, number];
export const load_file_batch: (a: number, b: number, c: number, d: number, e: number, f: number, g: number, h: number) => [number, number, number];
export const load_file_batch_with_text: (a: number, b: number, c: number, d: number, e: number, f: number, g: number, h: number, i: number, j: number) => [number, number, number];
export const commit_file_load: () => [number, number, number];
export const search_files: (a: number, b: number, c: number, d: number, e: number, f: number, g: number, h: number, i: number, j: number, k: number, l: number, m: number) => [number, number, number];
export const list_files_from_wasm: (a: number, b: number, c: number, d: number, e: number, f: number, g: number) => [number, number, number];
export const __wbindgen_malloc: (a: number, b: number) => number;
export const __wbindgen_realloc: (a: number, b: number, c: number, d: number) => number;
export const __wbindgen_free: (a: number, b: number, c: number) => void;
export const __wbindgen_exn_store: (a: number) => void;
export const __externref_table_alloc: () => number;
export const __wbindgen_externrefs: WebAssembly.Table;
export const __externref_table_dealloc: (a: number) => void;
export const __wbindgen_start: () => void;
